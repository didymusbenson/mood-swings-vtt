...
| F13 | ✅ | engine/UX | **Grief's mechanic didn't work — you couldn't play cards from the discard pile.** The engine fully supported `from: 'discard'` plays, but the UI had no entry point (the discard was view-only), so Grief/Angst/Harmony/Grace/Melancholy grants were unusable. | Added `queries.legalDiscardPlays(state, player, db)` (mirrors the engine's discard-play legality: discard-play grants, Melancholy permission + a normal play, Grace colour-grant; excludes Doubt-banned colours). The **discard pile now lights up when a discard-play is available**, and the inspector marks the legal cards playable — clicking one plays it from the discard (routing targeted cards through the normal flow with `from: 'discard'`). Also raised the floated info/Pass/Play controls' z-index so they stay clickable over the hand's outer cards (F11 follow-up). +2 engine tests; verified live end-to-end (log: "Player 1 plays Faith from the discard pile"; Discard 1→0). |
